...
def show_category(category_id):
    ...
    items = session.query(ItemTitle).filter_by(
        category_id=cat.id).order_by(asc(ItemTitle.name))
    return render_template(
        'category.html',
        categories=categories,
        category=cat,
...
def new_item(category_id):
    ...
    if request.method == 'POST':
        ...
        return render_template(
            'new_item.html', categories=categories, **field_vals)
    else:
        if category_id:
            cat_name = db.get_cat(category_id).name
            return render_template(
                'new_item.html', categories=categories, default_cat=cat_name)
        else:
...
def fbconnect():
    ...
    token_url = '''https://graph.facebook.com/oauth/access_token?grant_type=fb_exchange_token&client_id={id}
    &client_secret={secret}&fb_exchange_token={token}'''.format(
        id=app_id,
        secret=app_secret,
        token=access_token
        )
    ...
    userinfo_url = base_url + '?{token}&fields=name,id,email'.format(
        token=token)
    h = httplib2.Http()
    ...
    pic_url = base_url + '''/picture?{token}&
                    redirect=0&height=200&width=200'''.format(
        token=token)
    h = httplib2.Http()
...
def fbdisconnect():
    """FB disonnect functionality.
    Pairs with universal disconnect function."""
    facebook_id = login_session['facebook_id']
    access_token = login_session['access_token']
    url = 'https://graph.facebook.com/{id}/permissions?access_token={token}'
    url.format(
                id=facebook_id,
                token=access_token
                )
...
def category_items_json(category_id):
    ...
    return jsonify(
        Category=category.name, Items=[item.serialize for item in item_list])
